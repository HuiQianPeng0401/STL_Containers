#ifndef _PHQ_HASHTABLE_H_
#define _PHQ_HASHTABLE_H_
...
namespace phq
{
    ...
    struct hashtable_node
    {
        ...
        hashtable_node(hashtable_node &&node) : next(node.next), value(phq::move(node.value))
        {
            node.next = nullptr;
        }
    };
    ...
    struct ht_iterator_base : public phq::iterator<phq::forward_iterator_tag, T>
    {
        typedef phq::hashtable<T, Hash, KeyEqual> hashtable;
        typedef ht_iterator_base<T, Hash, KeyEqual> base;
        typedef phq::ht_iterator<T, Hash, KeyEqual> iterator;
        typedef phq::ht_const_iterator<T, Hash, KeyEqual> const_iterator;
    ...
    // local iterator
    template <class T>
    struct ht_local_iterator : public phq::iterator<phq::forward_iterator_tag, T>
    {
        typedef T value_type;
        typedef value_type *pointer;
        typedef value_type &reference;
    ...
    template <class T>
    struct ht_const_local_iterator : public phq::iterator<phq::forward_iterator_tag, T>
    {
        typedef T value_type;
        typedef const value_type *pointer;
        typedef const value_type &reference;
...
#define PRIME_NUM 44
    ...
    class hashtable
    {
        ...
        friend struct phq::ht_iterator<T, Hash, KeyEqual>;
        friend struct phq::ht_const_iterator<T, Hash, KeyEqual>;
    ...
    public:
        ...
        typedef phq::vector<node_ptr> bucket_type;
        ...
        typedef phq::ht_iterator<T, Hash, KeyEqual> iterator;
        typedef phq::ht_const_iterator<T, Hash, KeyEqual> const_iterator;
        typedef phq::ht_local_iterator<T> local_iterator;
        typedef phq::ht_const_local_iterator<T> const_local_iterator;
    ...
    public:
        ...
        template <class Iter, typename std::enable_if<
                                  phq::is_input_iterator<Iter>::value, int>::type = 0>
        hashtable(Iter first, Iter last,
                  size_type bucket_count,
                  const Hash &hash = Hash(),
                  const KeyEqual &equal = KeyEqual())
            : size_(phq::distance(first, last)), mlf_(1.0f), hash_(hash), equal_(equal)
        {
            init(std::max(bucket_count, static_cast<size_type>(phq::distance(first, last))));
        }
        ...
        hashtable(hashtable &&rhs) noexcept
            : bucket_size_(rhs.bucket_size_),
              ...
              equal_(rhs.equal_)
        {
            buckets_ = phq::move(rhs.buckets_);
            rhs.bucket_size_ = 0;
            rhs.size_ = 0;
            rhs.mlf_ = 0.0f;
        }
        ...
        {
            return emplace_multi(phq::forward<Args>(args)...);
        }

        template <class... Args>
        iterator emplace_unique_use_hint(const_iterator /*hint*/, Args &&...args)
        {
            return emplace_unique(phq::forward<Args>(args)...).first;
        }
        ...
        {
            return emplace_multi(phq::move(value));
        }
        ...
        {
            THORW_OUT_OF_RANGE_IF(ml != ml || ml < 0, "invalid hash load factor");
            mlf_ = ml;
        }
    ...
    private:
        ...
        // insert
        template <class InputIter>
        void copy_insert_multi(InputIter first, InputIter last, phq::input_iterator_tag);
        template <class ForwardIter>
        void copy_insert_multi(ForwardIter first, ForwardIter last, phq::forward_iterator_tag);
        template <class InputIter>
        void copy_insert_unique(InputIter first, InputIter last, phq::input_iterator_tag);
        template <class ForwardIter>
        void copy_insert_unique(ForwardIter first, ForwardIter last, phq::forward_iterator_tag);
    ...
    {
        auto np = create_node(phq::forward<Args>(args)...);
        try
        {
    ...
    hashtable<T, Hash, KeyEqual>::insert_unique_noresize(const value_type &value)
    {
        ...
        for (auto cur = first; cur; cur = cur->next)
        {
            if (is_equal(value_traits::get_key(cur->value), value_traits::get_key(value)))
                return phq::make_pair(iterator(cur, this), false);
        }
        // 让新节点成为链表的第一个节点
        ...
        buckets_[n] = tmp;
        ++size_;
        return phq::make_pair(iterator(tmp, this), true);
    }
    ...
    hashtable<T, Hash, KeyEqual>::erase_multi(const key_type &key)
    {
        ...
        if (p.first.node != nullptr)
        {
            ...
            return phq::distance(p.first, p.second);
        }
        return 0;
    }
            ...
            { // 如果出现相等的键值
                for (node_ptr second = first->next; second; second = second->next)
                {
                    if (!is_equal(value_traits::get_key(second->value), key))
                        return phq::make_pair(iterator(first, this), iterator(second, this));
                }
                for (auto m = n + 1; m < bucket_size_; ++m)
                { // 整个链表都相等，查找下一个链表出现的位置
                    if (buckets_[m])
                        return phq::make_pair(iterator(first, this), iterator(buckets_[m], this));
                }
                return phq::make_pair(iterator(first, this), end());
            }
        }
        return phq::make_pair(end(), end());
    }
    ...
    hashtable<T, Hash, KeyEqual>::equal_range_multi(const key_type &key) const
    {
        ...
        for (node_ptr first = buckets_[n]; first; first = first->next)
        {
            if (is_equal(value_traits::get_key(first->value), key))
            {
                ...
                {
                    if (!is_equal(value_traits::get_key(second->value), key))
                        return phq::make_pair(M_cit(first), M_cit(second));
                }
                for (auto m = n + 1; m < bucket_size_; ++m)
                { // 整个链表都相等，查找下一个链表出现的位置
                    if (buckets_[m])
                        return phq::make_pair(M_cit(first), M_cit(buckets_[m]));
                }
                return phq::make_pair(M_cit(first), cend());
            }
        }
        return phq::make_pair(cend(), cend());
    }
    ...
    hashtable<T, Hash, KeyEqual>::equal_range_unique(const key_type &key)
    {
        ...
        for (node_ptr first = buckets_[n]; first; first = first->next)
        {
            ...
            {
                if (first->next)
                    return phq::make_pair(iterator(first, this), iterator(first->next, this));
                for (auto m = n + 1; m < bucket_size_; ++m)
                { // 整个链表都相等，查找下一个链表出现的位置
                    if (buckets_[m])
                        return phq::make_pair(iterator(first, this), iterator(buckets_[m], this));
    ...
    hashtable<T, Hash, KeyEqual>::equal_range_unique(const key_type &key) const
    {
        ...
        for (node_ptr first = buckets_[n]; first; first = first->next)
        {
            ...
            {
                if (first->next)
                    return phq::make_pair(M_cit(first), M_cit(first->next));
                for (auto m = n + 1; m < bucket_size_; ++m)
                { // 整个链表都相等，查找下一个链表出现的位置
                    if (buckets_[m])
                        return phq::make_pair(M_cit(first), M_cit(buckets_[m]));
        ...
        swap(hashtable &rhs) noexcept
    {
        if (this != &rhs)
        {
            ...
            phq::swap(bucket_size_, rhs.bucket_size_);
            phq::swap(size_, rhs.size_);
            phq::swap(mlf_, rhs.mlf_);
            phq::swap(hash_, rhs.hash_);
            phq::swap(equal_, rhs.equal_);
        }
    }
    ...
    template <class T, class Hash, class KeyEqual>
    template <class InputIter>
    void hashtable<T, Hash, KeyEqual>::copy_insert_multi(InputIter first, InputIter last, phq::input_iterator_tag)
    {
        rehash_if_need(phq::distance(first, last));
    ...
    template <class T, class Hash, class KeyEqual>
    template <class ForwardIter>
    void hashtable<T, Hash, KeyEqual>::copy_insert_multi(ForwardIter first, ForwardIter last, phq::forward_iterator_tag)
    {
    ...
    template <class T, class Hash, class KeyEqual>
    template <class InputIter>
    void hashtable<T, Hash, KeyEqual>::copy_insert_unique(InputIter first, InputIter last, phq::input_iterator_tag)
    {
        rehash_if_need(phq::distance(first, last));
        for (; first != last; ++first)
    ...
    template <class T, class Hash, class KeyEqual>
    template <class ForwardIter>
    void hashtable<T, Hash, KeyEqual>::copy_insert_unique(ForwardIter first, ForwardIter last, phq::forward_iterator_tag)
    {
        size_type n = phq::distance(first, last);
        rehash_if_need(n);
        for (; n > 0; --n, ++first)
    ...
    hashtable<T, Hash, KeyEqual>::insert_node_unique(node_ptr np)
    {
        ...
        if (cur == nullptr)
        {
            buckets_[n] = np;
            ++size_;
            return phq::make_pair(iterator(np, this), true);
        }
        for (; cur; cur = cur->next)
        {
            if (is_equal(value_traits::get_key(cur->value), value_traits::get_key(np->value)))
    ...
    bool hashtable<T, Hash, KeyEqual>::equal_to_multi(const hashtable &other)
    {
        ...
        for (auto f = begin(), l = end(); f != l;)
        {
            ...
            if (phq::distance(p1.first, p1.last) != phq::distance(p2.first, p2.last) || !std::is_permutation(p1.first, p2.last, p2.first, p2.last))
                return false;
            f = p1.last;
        }
        return true;
