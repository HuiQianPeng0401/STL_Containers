#ifndef _PHQ_UNORDERED_MAP_H_
#define _PHQ_UNORDERED_MAP_H_
...
// 参数一代表键值类型，参数二代表实值类型，参数三代表哈希函数，缺省使用 std::hash
// 参数四代表键值比较方式，缺省使用 std::equal_to
template <class Key, class T, class Hash = std::hash<Key>, class KeyEqual = std::equal_to<Key>>
class unordered_map
{
private:
  // 使用 hashtable 作为底层机制
  typedef hashtable<phq::pair<const Key, T>, Hash, KeyEqual> base_type;
  base_type ht_;
...
public:
  ...
  unordered_map(InputIterator first, InputIterator last,
                ...
                const Hash& hash = Hash(),
                const KeyEqual& equal = KeyEqual())
    : ht_(std::max(bucket_count, static_cast<size_type>(phq::distance(first, last))), hash, equal)
  {
  ...
  { return ht_.emplace_unique(phq::forward<Args>(args)...); }

  template <class ...Args>
  iterator emplace_hint(const_iterator hint, Args&& ...args)
  { return ht_.emplace_unique_use_hint(hint, phq::forward<Args>(args)...); }
  ...
  { return ht_.emplace_unique(phq::move(value)); }

  iterator insert(const_iterator hint, const value_type& value)
  { return ht_.insert_unique_use_hint(hint, value); }
  iterator insert(const_iterator hint, value_type&& value)
  { return ht_.emplace_unique_use_hint(hint, phq::move(value)); }
  ...
  mapped_type& at(const key_type& key)
  {
    ...
    THORW_OUT_OF_RANGE_IF(it.node == nullptr, "unordered_map<Key, T> no such element exists");
    return it->second;
  }
  ...
  mapped_type& operator[](key_type&& key)
  {
    iterator it = ht_.find(key);
    if (it.node == nullptr)
      it = ht_.emplace_unique(phq::move(key), T{}).first;
    return it->second;
  }
...
public:
  ...
  { return ht_.emplace_multi(phq::forward<Args>(args)...); }

  template <class ...Args>
  iterator emplace_hint(const_iterator hint, Args&& ...args)
  { return ht_.emplace_multi_use_hint(hint, phq::forward<Args>(args)...); }
  ...
  { return ht_.emplace_multi(phq::move(value)); }

  iterator insert(const_iterator hint, const value_type& value)
  { return ht_.insert_multi_use_hint(hint, value); }
  iterator insert(const_iterator hint, value_type&& value)
  { return ht_.emplace_multi_use_hint(hint, phq::move(value)); }
